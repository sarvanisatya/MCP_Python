...
L = []
L.append([s,0,0,[s]]) # maintains a list of labels that  follows FIFO priority rule. 

temp=[]
while L:
    cur=L[0]   # [node#id, cost label, time label]
        #print cur[3]
    del L[0]
    for j in G1.neighbors_iter(cur[0]):  # For Loop XYZ
            ...
            #trace = []
            temp=[]
            temp=[j,cur[1]+G1[cur[0]][j]['cost'],cur[2]+G1[cur[0]][j]['time'],[]]
            #print temp[3], trace#print len(D[j])
            dom_list = []
            
            for k in range(0,len(D[j])):
                #print k
                if temp[1]>D[j][k][1] and temp[2]>D[j][k][2]:       #exlude dominated labels
                    ind=1
                    break
                elif temp[1]==D[j][k][1] and temp[2]==D[j][k][2]:   # exclude labels already found
                    ind=1
                    break
                elif temp[1]<D[j][k][1] and temp[2]<D[j][k][2]:     
                    ind=0
                    dom_list.append(k)
